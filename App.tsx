...
import { analyzeStock, analyzeAllModes, sanitizeApiKey } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Auto-clean key input before saving
    const cleanKey = sanitizeApiKey(inputKey);
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 md:h-20 flex items-center justify-between">
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[8px] font-black text-green-500/60 tracking-widest uppercase">Key: {userKey ? `...${userKey.slice(-5)}` : 'UNKNOWN'}</span>
            </div>
          </div>
      ...
      <main className="relative z-10 pt-12 md:pt-20 pb-40 px-4 md:px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div id="result-section" className="mt-20 md:mt-32">
            {error && (
              <div className={`p-6 md:p-8 border rounded-2xl text-center font-black uppercase tracking-[0.2em] text-xs flex flex-col items-center gap-4 ${
                ...
                {(error.includes("API Key") || error.includes("API key")) && (
                  <button 
                    ...
                    className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest border transition-all ${
                       isDarkMode ? 'border-red-500 text-red-400 hover:bg-red-500 hover:text-white' : 'border-red-600 text-red-600 hover:bg-red-600 hover:text-white'
                    }`}
                  >
                    Reset API Key
                  </button>
